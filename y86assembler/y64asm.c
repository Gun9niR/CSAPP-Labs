...
void add_reloc(char *name, bin_t *bin)
{
    ...
    reloc_t *newReloc = malloc(sizeof(reloc_t));
    newReloc->y64bin = bin;
    newReloc->name = name;
...
parse_t parse_reg(char **ptr, regid_t *regid)
{
    ...
    /* find register */
    const reg_t *regPtr = find_register(*ptr);
    if (!regPtr) {
...
parse_t parse_symbol(char **ptr, char **name)
{
    ...
    while (!IS_END(c) && !IS_BLANK(c) && (IS_LETTER(c) || IS_DIGIT(c))) {
        ++c;
        ++len;
    }
...
parse_t parse_digit(char **ptr, long *value)
{
    /* skip the blank and check */
    SKIP_BLANK(*ptr);
    if (IS_END(*ptr) || (!IS_DIGIT(*ptr) && **ptr != '-')) {
        return PARSE_ERR;
    }
...
parse_t parse_label(char **ptr, char **name)
{
    ...
    /* allocate name and copy to it */
    char *c = *ptr;     // ptr stores the start of the string, c the end
    int len = 0;
    while (!(IS_END(c)) && !IS_BLANK(c)) {
        if (*c == ':') {
            /* set 'ptr' and 'name' */
            *name = malloc(len + 1);
            strncpy(*name, *ptr, len);
...
void set_n_byte(byte_t *code, long value, int n) {
    for (int i = 0; i < n; ++i) {
        code[i] = value & 0xFF;
        value >>= 8;
    }
...
{
    // change the bin_t and line->type field in line
        ...
        switch (HIGH(inst->code)) {
            ...
            case I_ALU:
                if (parse_reg(&ptr, &regA) == PARSE_ERR ||
                    ...
                    parse_reg(&ptr, &regB) == PARSE_ERR) {
                    return TYPE_ERR;
                }
                line->y64bin.codes[1] = HPACK(regA, regB);
                break;
            case I_IRMOVQ:
                if ((parseRes = parse_imm(&ptr, &name, &value)) == PARSE_ERR ||
                    parse_delim(&ptr, ',') == PARSE_ERR ||
                    parse_reg(&ptr, &regB) == PARSE_ERR) {
                        return TYPE_ERR;
                    }
                line->y64bin.codes[1] = HPACK(REG_NONE, regB);
                if (parseRes == PARSE_DIGIT) {
                    // put the number
                    set_n_byte(line->y64bin.codes + 2, value, 8);
                } else {
                    // set relocation
            ...
            case I_RMMOVQ:
                if (parse_reg(&ptr, &regA) == PARSE_ERR ||
                    parse_delim(&ptr, ',') == PARSE_ERR ||
                    parse_mem(&ptr, &value, &regB) == PARSE_ERR) {
                        return TYPE_ERR;
                    }
                line->y64bin.codes[1] = HPACK(regA, regB);
                set_n_byte(line->y64bin.codes + 2, value, 8);
            ...
            case I_CALL:
                ...
                if (parseRes == PARSE_DIGIT) {
                    set_n_byte(line->y64bin.codes + 1, value, 8);
                } else {
                    add_reloc(name, &line->y64bin);
                }
                break;
            ...
            case I_DIRECTIVE:
                switch(LOW(inst->code)) {
                    case D_DATA:
                        if ((parseRes = parse_data(&ptr, &name, &value)) == PARSE_ERR) {
                            return TYPE_ERR;
                        }
                        if (parseRes == PARSE_DIGIT) {
                            set_n_byte(line->y64bin.codes, value, inst->bytes);
                        } else {
                            add_reloc(name, &line->y64bin);
                        }
                        break;
...
int relocate(void)
{
    ...
    while (rtmp) {
        ...
        switch (HIGH(rtmp->y64bin->codes[0])) {
            case I_IRMOVQ:
                set_n_byte(rtmp->y64bin->codes + 2, value, 8);
                break;
            case I_JMP:
            case I_CALL:
                set_n_byte(rtmp->y64bin->codes + 1, value, 8);
                break;
            case I_DIRECTIVE:
                set_n_byte(rtmp->y64bin->codes, value, 8);
                break;
            default: 
                return -1;
        }
...
int binfile(FILE *out)
{
    ...
    byte_t buffer[MAX_INSLEN];
    memset(buffer, 0, MAX_INSLEN);
    size_t lastAddr = 0;

    line_t *line = line_head->next;
    while (line) {
        memcpy(buffer + line->y64bin.addr, line->y64bin.codes, line->y64bin.bytes);
        if (line->y64bin.bytes && line->y64bin.addr + line->y64bin.bytes > lastAddr) {
            lastAddr = line->y64bin.addr + line->y64bin.bytes;
        }
        line = line->next;
    }
    
    /* binary write y64 code to output file (NOTE: see fwrite()) */
    if (fwrite(buffer, sizeof(byte_t), lastAddr, out) != lastAddr) {
        return -1;
    }
    return 0;
}
