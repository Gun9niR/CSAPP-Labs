...
// output state name based on its type
...
mem_t *init_mem(int len)
{
    ...
    // round len up to the multiple of BLK_SIZE
    len = ((len+BLK_SIZE-1)/BLK_SIZE)*BLK_SIZE;
    m->len = len;
    // memory is already set to zero
...
cc_t compute_cc(alu_t op, long_t argA, long_t argB, long_t val)
{
    ...
    long_t signA = argA >> 63 & 1;
    long_t signB = argB >> 63 & 1;
    long_t signV = val >> 63 & 1;
    switch(op) {
        case A_ADD: 
            ovf = (signA == signB) && (signA != signV);
            break;
        // tricky: sub is argB - argA
        case A_SUB:
            ovf = (signA != signB) && (signA == signV);
            break;
        default: 
...
stat_t nexti(y64sim_t *sim)
{
    ...
    byte_t register_ab = REG_NONE;
    itype_t icode;
    alu_t ifun;
    long_t next_pc = sim->pc;
    
    byte_t register_a = REG_NONE;
    byte_t register_b = REG_NONE;
    long_t immediate;
    ...
    icode = GET_ICODE(codefun); // instruction code (first 4 bit)
    ifun = GET_FUN(codefun);    // func code (next 4 bit)
    next_pc++;
    ...
    switch (icode) {
        ...
        case I_ALU: case I_PUSHQ: case I_POPQ:
            if (!get_byte_val(sim->m, next_pc, &register_ab)) {
                err_print("PC = 0x%lx, Invalid instruction address", sim->pc);
                return STAT_ADR;
            }
            register_a = GET_REGA(register_ab);
            register_b = GET_REGB(register_ab);
            ++next_pc;
            break;
    };
    ...
    case I_RRMOVQ: { /* 2:x regA:regB */
        bool_t flag = cond_doit(sim->cc, ifun);
        if (flag == TRUE) {
            long_t val = get_reg_val(sim->r, register_a);
            set_reg_val(sim->r, register_b, val);
        }
        sim->pc = next_pc;
        break;
    }
    ...
    case I_IRMOVQ: {/* 3:0 F:regB imm */
        set_reg_val(sim->r, register_b, immediate);
        sim->pc = next_pc;
        break;
    }
    ...
    case I_RMMOVQ: {/* 4:0 regA:regB imm */
        long_t val = get_reg_val(sim->r, register_a);
        long_t dest = get_reg_val(sim->r, register_b) + immediate;

        if (!set_long_val(sim->m, dest, val)) {
            err_print("PC = 0x%lx, Invalid memory address", sim->pc);
            return STAT_ADR;
        }
        sim->pc = next_pc;
        break;
    ...
    case I_MRMOVQ: {/* 5:0 regB:regA imm */
        long_t from = get_reg_val(sim->r, register_b) + immediate;
        long_t val = 0;
        if (!get_long_val(sim->m, from, &val)) {
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc, from);
            return STAT_ADR;
        }
        set_reg_val(sim->r, register_a, val);
        sim->pc = next_pc;
        break;
    }
    case I_ALU: {/* 6:x regA:regB */
        long_t val1 = get_reg_val(sim->r, register_a);
        long_t val2 = get_reg_val(sim->r, register_b);
        long_t res = compute_alu(ifun, val1, val2);
        set_reg_val(sim->r, register_b, res);
        sim->cc = compute_cc(ifun, val1, val2, res);
        sim->pc = next_pc;
        break;
    }
    ...
    case I_RET: {/* 9:0 */
        long_t ret_address;
        long_t rsp = get_reg_val(sim->r, REG_RSP);
        // acquire return address
        if (!get_long_val(sim->m, rsp, &ret_address)) {
            err_print("PC = 0x%lx, Invalid return address", sim->pc);
            return STAT_ADR;
        }
        rsp += 8;
        set_reg_val(sim->r, REG_RSP, rsp);
